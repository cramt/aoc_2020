...
impl Day5 {
    fn parse_identifiers(&self) -> Vec<(&str, &str)> {
        self.input().split_ascii_whitespace().filter(|x| *x != "")
            .map(|x| {
    ...
    fn parse_positions(&self) -> Vec<(usize, usize, usize)> {
        fn convert(chars: Vec<char>, up: char, down: char, starting_range: CuttableRange) -> usize {
            chars.into_iter().fold(starting_range, |acc, x| {
                let cut = acc.cut();
                match x {
                    k if (k == up) => cut.1,
                    k if (k == down) => cut.0,
        ...
        self.parse_identifiers().into_iter()
            .map(|(row, column)| (row.chars().collect::<Vec<char>>(), column.chars().collect::<Vec<char>>()))
            .map(|(row, column)| (
...
impl Day<usize> for Day5 {
    fn part1(&self) -> usize {
        let mut ids = self.parse_positions().into_iter().map(|(_, _, id)| id).collect::<Vec<usize>>();
        ids.sort_by(|b, a| a.cmp(b));
        return *ids.get(0).unwrap();
    }

    fn part2(&self) -> usize {
        let (row, columns) = self.parse_positions().into_iter().fold(HashMap::new(), |mut acc: HashMap<usize, Vec<usize>>, (row, column, _)| {
            ...
            .unwrap();
        let column = columns.into_iter().fold((0..=7).into_iter().collect::<HashSet<usize>>(), |mut acc, x| {
            acc.remove(&x);
            acc
        }).into_iter().find(|_| true).unwrap();
        (row * 8) + column
    }
